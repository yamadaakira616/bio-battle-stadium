...
const cardMap = Object.fromEntries([...STICKERS, ...FUSIONS].map(s => [s.id, s]));
...
export default function TeamSelectScreen({ state, nation, onBack, onConfirm }) {
  ...
  function getInitialSelection() {
    ...
    SERIES_ORDER.forEach((series, idx) => {
      ...
      const lastCard = lastTeam[idx] ? cardMap[lastTeam[idx]] : null;
        ...
        {SERIES_ORDER.map(series => {
          ...
          return (
            ...
            >
              ...
              {/* Card Scroll */}
              {(() => {
              const pool = hasCards ? owned : [cardMap[STARTERS[series]]].filter(Boolean);
              return (
